...
func try_snap_to_points(world_pos: Vector2, snap_points: Array[Vector2], snap_tolerance: float, world_to_screen_func: Callable) -> Vector2:
	...
	if not world_to_screen_func or not world_to_screen_func.is_valid():
		...
		# When we have a world_to_screen converter, compare in screen (pixel) space.
		# Use SNAP_DISTANCE (pixels) as the pixel tolerance.
